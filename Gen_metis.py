...
# funziona solamente con labelificatori tensorflow
...
from Utils_gen_metis import *
import sys
import os
import time
from Classifiers import TF_LeNet1
...
def run_dlfuzz(label, model1, input_tensor, starting_seeds, imgs_to_sample, run_folder):
    ...
    while SEEDCOUNT < len(img_paths) and adversial_num < imgs_to_sample:
        ...
        orig_label = label
        #img_name = 'seed_'+str(img_paths[i])+'_label_'+str(orig_label)
        img_name = 'seed_' + str(SEEDCOUNT) + '_label_' + str(orig_label)
        #tmp_img = reshape(x_test[int(img_paths[i])])
        ...
        while len(img_list) > 0:
            ...
            for iters in range(iteration_times):
                ...
                if label1 != orig_label:
                    ...
                    save_img = os.path.join(run_folder, img_name + '_' + str(get_signature()))

                    np.save(save_img, gen_img)

                    # TODO: remove the following
                    prova= np.load(save_img + '.npy')
                    predx = model1.predict(prova)
                    labelx = np.argmax(predx[0])
                    assert(labelx == label1)
    ...
    with open(summary_file, 'w') as f:
        f.write(f"Classifier used: {model1.name}\n")
